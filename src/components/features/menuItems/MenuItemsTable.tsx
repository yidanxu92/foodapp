...
const MenuItemsTable = ({ menuItems, onDelete }: MenuItemsTableProps) => {
  ...
  return (
    <Table aria-label="Menu Items Table" classNames={{ th: "text-md", td: "text-md text-gray-600 border-b" }}>
      <TableHeader>
        ...
        <TableBody>
          {menuItems.map((menuItem) => (
            <TableRow key={menuItem._id}>
              ...
              <TableCell>{categories.find(c => c._id === menuItem.category.toString())?.name}</TableCell>
